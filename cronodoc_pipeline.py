...
try:
    ...
    compact_visual = ' '.join(str(detected_visual).split()[:20])
    render_prompt = (
        f"Museum reconstruction of {safe_value(sample, 'artifact_title', 'title')}. "
        f"Era: {safe_value(sample, 'historic_era', 'historical_era', 'era')}. "
        f"Visuals: {compact_visual}. Realistic materials, natural lighting."
    )
